...
class ANN:
    ...
    def Backpropagation(self, y, output, intermediate):
        ...
        for i in reversed(range(len(deltas)-1)):
            ...
            Dw += np.ones(np.array(self.weights).shape)*Lambda*(self.weights)
            Db += np.ones(np.array(self.bias).shape)*Lambda*(self.bias)
        return Dw, Db
...
for i in range(len(E1)):
    L1.append(i)
plt.scatter(L1,E1,color='BLUE')
# print(E1,L1)

L2=[]
for i in range(len(E2)):
    L2.append(i)
plt.scatter(L2,E2,color='GREEN',s=100)
print("MAPE test is:",E2,L2)


plt.xlabel("Epochs")
plt.ylabel("MAPE")
plt.legend(['Training','Validaiton','Test'])
plt.show()
